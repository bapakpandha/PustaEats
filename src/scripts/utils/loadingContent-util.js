function showLoadingContent(
  ...
  function js() {
    const text = 'PustaEats sedang memuat';
...
function showEmptyContent(
  element = document.querySelector('main'),
  message = 'Tidak Ada Data Yang Ditampilkan. Daftar Restoran Kosong.'
) {
  const html = document.createElement('div');
  html.className = 'container-Empty';
  html.innerHTML = `<div class="loader" style="text-align:center;color:#365486;display:flex;flex-direction: column;align-items: center;justify-content: center;"><img src="./images/empty.gif" alt="Tidak Ada Kontent" style="max-width: 100%;"><h1>${message}</h1></div><style>.container .list_notes div.content {display: block;}</style>`;
  element.innerHTML = '';
  element.appendChild(html);
}
...
export { showEmptyContent, showLoadingContent, loadContent };